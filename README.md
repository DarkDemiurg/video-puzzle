...
- Звук: авто (первый со звуком) или выбранный слот; опционально нормализация громкости
- Качество выхода: быстрое / обычное / высокое / **как оригинал**
- Кодек: авто (NVENC / Intel QSV / CPU) или вручную
- FPS берётся из исходников (максимальный), иначе 30
- Зазор между ячейками, кроп и поворот в окне разметки
- Кадр склейки — превью готовой мозаики без полного кодирования
- Проект `.vproj`: сетка, файлы, фрагменты, качество; размер окна и последняя папка запоминаются
- После сборки можно открыть файл или папку; перед стартом показывается оценка размера
...
Слоты можно менять местами перетаскиванием. Сборку можно остановить. Неполный выходной файл при отмене удаляется.
