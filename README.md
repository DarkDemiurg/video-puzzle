...
На Linux для Qt/PySide6 также нужны системные библиотеки (Ubuntu/Debian):

```bash
sudo apt install libxcb-cursor0 libxkbcommon-x11-0 libegl1 libopengl0
```

### Если после запуска «ничего не происходит»

В релизе **v0.1.0** PyApp при первом запуске создаёт виртуальное окружение и **скачивает PySide6 из интернета** (сотни МБ, 5–15 минут без явного прогресса). Консоль может закрыться, а окно появится только после установки.

- Убедитесь, что есть интернет и подождите.
- Запускайте из терминала в распакованной папке: `./video-puzzle`
- Логи и кэш: `~/.local/share/pyapp/video-puzzle/`
- Начиная с **v0.1.1** зависимости встроены в бинарник — первый запуск быстрый и не требует сети.
